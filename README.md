...
- 每个玩家绑定一个 `factionId`，可以是 `1-40` 普通国家，也可以是 `101-200` 叛乱国家。
...
管理员入口 `?admin=1` 中提供“Azgaar 风格生成”：基于 seed 生成高度图、陆海、山脉、湿度、温度、生物群系和河流，再在宜居陆地上生成 40 个国家。普通玩家默认仍使用本机保存地图；没有保存地图时使用内置中国地图兜底。
